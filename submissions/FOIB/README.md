...
| Underactuated tendon coupling | Two MuJoCo `<fixed>` tendons (`couple_ab`, `couple_bc`) model a common-pulley differential between the three fingers (stiffness 0.3 N/m, 67× weaker than actuators); spatial tendon `grip_ring` renders the triangular fingertip ring visually |
| Type-annotated API | `PhaseController.__init__`, `reset`, `step`, `overlay_info`, `grasp_quality`, `_solve_ik` carry full `from __future__ import annotations` PEP 563 type hints; 6-test pytest suite in `tests/` |
...
pip install pytest          # for the test suite (not required for benchmark)
```
...
Six tests cover `PhaseController` reset, step return types, `grasp_quality` bounds,
`overlay_info` key completeness, shell-shard underground state, and `egg_qpos_adr`
joint-index correctness.  All tests pass in < 1 s without launching any video window.
