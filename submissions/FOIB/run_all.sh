...
# Produces 3 submission videos + 7 CSVs (seed 42).
...
echo "==> two-egg easy (10 eps)  [CSV only]"
$PYTHON "$DIR/video/record_demo.py" \
    --two-egg --episodes 10 --tier easy --seed 42 \
    --out /tmp/_te_easy_scratch.mp4 --log "$DIR/results_two_egg_easy.csv"
rm -f /tmp/_te_easy_scratch.mp4

echo "==> two-egg medium (10 eps)  [CSV only]"
$PYTHON "$DIR/video/record_demo.py" \
...
echo "==> two-egg stress (10 eps)  [CSV only]"
$PYTHON "$DIR/video/record_demo.py" \
    --two-egg --episodes 10 --tier stress --seed 42 \
    --out /tmp/_te_stress_scratch.mp4 --log "$DIR/results_two_egg_stress.csv"
rm -f /tmp/_te_stress_scratch.mp4
