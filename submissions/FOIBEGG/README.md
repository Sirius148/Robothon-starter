...
| Difficulty gradient | `min_egg_sep` drives 10 → 6 → 4 → 2 success rate; controller and thresholds never modified |
...
| `benchmark_two_egg_medium.mp4` | Two-egg static (mode 2), 75 mm sep | Distractor robustness at intended operating point (6/10); failure gallery: `DISTRACTOR_DISTURBED` + `DROPPED` |
| `benchmark_two_egg_extreme.mp4` | Two-egg static (mode 2), 66 mm sep | Full failure taxonomy across 8 failures; overhead view shows why proximity drives the drop from 6 → 2 |
...
| `benchmark_two_egg_medium.mp4` | two-egg static, 75 mm sep, seed 42 | **6 / 10** |
| `benchmark_two_egg_extreme.mp4` | two-egg static, 66 mm sep, seed 42 | **2 / 10** |
...
Success   10/10 ██████████  6/10 ██████  4/10 ████  2/10 ██
```

Performance degrades monotonically as the distractor enters the arm's finger-sweep
corridor. At 80 mm the sweep never reaches egg2; at 66 mm it does in 8 of 10
...
| two-egg (static) | medium | 10 | **6/10** (60 %) | 3 | 1 | — |
| two-egg (static) | stress | 10 | **4/10** (40 %) | 4 | 2 | — |
| two-egg (static) | extreme | 10 | **2/10** (20 %) | 5 | 3 | — |
...
| medium | 75 mm | **6 / 10** | 3 | 1 | 4 |
| stress | 70 mm | **4 / 10** | 4 | 2 | 6 |
| extreme | 66 mm | **2 / 10** | 5 | 3 | 8 |

The monotonic curve (10 → 6 → 4 → 2) confirms that `min_egg_sep` is the
controlling variable. At 80 mm the arm's finger sweep never reaches the
distractor; at 66 mm it does in 8 of 10 randomly seeded placements. Failures
